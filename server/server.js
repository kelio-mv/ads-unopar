...
class App {
  ...
  async run() {
    ...
    const shouldBeLocked = this.isDowntime() || this.isDailyLimitReached();
    this.setLocked(shouldBeLocked);
  ...
  async syncSystemTime() {
    ...
    const [dateString, timeString] = this.stringifyDateObject(date);
